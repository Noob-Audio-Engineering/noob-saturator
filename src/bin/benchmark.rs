...
//!
//! `cargo run --release --features plugin --bin benchmark -- --wav DIR` writes the stimulus
...
fn render_report(sections: &[Section], factors: &str, probe: Option<String>) -> String {
    ...
    match probe {
        ...
        None => out.push_str(
            "`cargo run --release --features plugin --bin benchmark -- --wav DIR` writes the stimulus and the \
